...
```bash
python run.py "data/input/material_cert.pdf" "data/output/composition.csv"
```

### Example 2: Batch processing (shell script)

```bash
for pdf in data/input/*.pdf; do
    name=$(basename "$pdf" .pdf)
    python run.py "$pdf" "data/output/${name}_composition.csv"
done
```

### Example 3: High-resolution processing

```bash
python src/main.py \
  --input "data/input/material.pdf" \
  --output "data/output/highres.csv" \
  --dpi 400 \
  --log "data/output/extraction.log"
```
...
- **OCR Accuracy**: Depends on PDF/image quality. Poor quality scans may produce errors.
- **Table Formats**: Works best with standard tabular formats. Complex layouts may need adjustment.
- **Language**: Currently optimized for English. Other languages may require language packs.
- **Handwriting**: Does not support handwritten text.

## Contributing

To improve the system:
1. Enhance OCR preprocessing for better accuracy
2. Add support for additional table formats
3. Improve error correction algorithms
4. Add support for more element symbols
5. Enhance value validation rules
...
3. Verify Tesseract OCR installation
4. Ensure PDF quality is sufficient (300+ DPI)
